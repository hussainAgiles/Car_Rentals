...
const ReservationSummary = ({reservation}: any) => {
  const calculateSubtotal = () => {
    const rentalPrice = reservation?.rental_price || 0;
    const addonTotalPrice = reservation?.addon_total_price || 0;
    const vatAmount =
      (rentalPrice + addonTotalPrice) * (reservation?.vat / 100);
    return rentalPrice + addonTotalPrice + vatAmount + reservation?.vmc;
  };

  const subtotal = calculateSubtotal();
  console.log(subtotal);
  const totalDue = subtotal - (reservation?.balance_amount || 0);
  const calculateRentPeriod = (pickup_date: string | number | Date, dropoff_date: string | number | Date) => {
  ...
  const totalDays = calculateRentPeriod(reservation?.pickup_date, reservation?.dropoff_date);
  ...
  return (
    <View style={styles.container}>
     ...
     <Text style={styles.title}>{totalDays} {totalDays >= 1 ? 'Day' : 'Days'}</Text>
     </View>
      ...
      <View style={styles.section}>
        ...
        <Text style={styles.price}>
          {reservation?.addon_total_price.toFixed(2)} AUD
        </Text>
      </View>
